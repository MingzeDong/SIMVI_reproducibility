"""PyTorch module for SIMVI graph only for modeling cellular interaction variation."""
...
class SimVIGOModule(nn.Module):
    ...
    def __init__(
        ...
        self.gat_mean = GATv2Conv(in_channels=n_spatial,out_channels=n_spatial,add_self_loops=False)
        self.gat_var = GATv2Conv(in_channels=n_spatial,out_channels=n_spatial,add_self_loops=False)
    ...
    def loss(
        ...
        kl_local_for_warmup = kl_divergence_zgat
        kl_local_no_warmup = kl_divergence_l

        weighted_kl_local = self.kl_gatweight * kl_divergence_zgat + kl_local_no_warmup

        loss = torch.mean(reconst_loss + weight * weighted_kl_local) 
